...
from src.ai.promt.chart_spec_promt import build_spec_prompt
...
class SpecGenerator:
    ...
    def generate_spec(
        ...
        if not self.enabled:
            raise RuntimeError(f"[SPEC_GEN] Purpose '{self.PURPOSE}' not enabled for tenant={TenantId}")
        ...
        try:
            ...
            logger.info(f"[SPEC_GEN][tenant={TenantId}][session={SessionId}] Success. Spec: {json.dumps(spec)}")
            return spec

        except Exception as e:
            logger.error(f"[SPEC_GEN][tenant={TenantId}][session={SessionId}] Failed: {e}", exc_info=True)
            raise RuntimeError(f"Spec generation failed: {e}")